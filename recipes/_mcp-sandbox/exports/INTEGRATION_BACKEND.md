...
2. POST `{ "data": [<input>] }` to `${ AI_GRADIO_URL }/call/predict` — returns an `event_id`.
3. GET `${ AI_GRADIO_URL }/call/predict/<event_id>` (SSE) and read the final
   `process_completed` event: `output.data[0]` is the AI result. Asset MIME: `application/json`.
4. When the URL changes (Colab restart), update the env var — team channel `#ai-endpoint`.
...
ai:
  gradio:
    url: ${AI_GRADIO_URL:}    # empty default -> /api/ai/* returns 503 with a clear error body
```
...
- **URL refresh procedure**: AI team posts the new URL to `#ai-endpoint` + updates the deployment env var. Backend retries failed requests once after a 30s delay.
...
2. Tag the AI subtree (e.g. `_mcp-sandbox-v0.1.0`) — GitLab tag if this lives in a SSAFY monorepo at `<repo>/ai/`
