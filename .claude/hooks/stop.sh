...
if [ -f "$LOG_FILE" ] && [ -s "$LOG_FILE" ]; then
  ...
  if uv run python -m compileall -q . 2>&1 | head -5; then
  ...
  if uv run python scripts/smoke_test.py 2>&1 | tail -3; then
    echo "[hook:stop] smoke_test: OK" >&2
  else
    echo "[hook:stop] smoke_test: FAILED (non-blocking)" >&2
  fi
  ...
  # Formatter recommendation (not auto-run)
  ...
  # Still refresh context pack (cheap)
  uv run python scripts/make_context_pack.py 2>&1 || true
fi
