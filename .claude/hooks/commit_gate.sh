...
CMD=$(printf '%s' "$INPUT" | python -c "import sys,json;print(json.loads(sys.stdin.read() or '{}').get('tool_input',{}).get('command','') if sys.stdin.isatty()==False else '')" 2>/dev/null || echo "")
...
VERDICT=$(python -c "import json;print(json.load(open(r'$MARKER',encoding='utf-8')).get('verdict',''))" 2>/dev/null || echo "")
...
MARKER_TS=$(python -c "import os,sys;print(int(os.path.getmtime(r'$MARKER')))" 2>/dev/null || echo 0)
STALE_FILES=""
while IFS= read -r f; do
    [ -z "$f" ] && continue
    [ -f "$REPO_ROOT/$f" ] || continue
    FILE_TS=$(python -c "import os,sys;print(int(os.path.getmtime(r'$REPO_ROOT/$f')))" 2>/dev/null || echo 0)
