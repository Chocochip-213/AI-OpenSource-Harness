...
- **[v4] v3 DWPose hull pre-filter 롤백 + SAM2 Image Predictor 마스크 엔진 추가** — v3 hull pre-filter가 마스크를 convex hull 형태(사각형)로 강제하여 의류 윤곽 손실. v2 SegFormer 파이프라인(HULL_MASK=False, DILATION_ITER=3, CONSENSUS_MODE="interior", EDGE_DILATION=True)으로 복원. 동시에 `MASK_ENGINE` 파라미터 추가: `"segformer"`(기본, v2 동작) / `"sam2"`(SAM2 Image Predictor per-frame). v3 코드(hull pre-filter, hull consensus)는 삭제하지 않고 비활성화만 — 수동 활성화 가능.
- **[v4] SAM2 Image Predictor per-frame** — 이전 SAM2 시도는 (1) Video Predictor(negative point 무시), (2) Image→Video 전파(drift)만 시도. **Image Predictor per-frame은 시도한 적 없음** — negative point가 frame-level에서 작동하므로 얼굴 제외 가능. DWPose keypoints로 포인트 프롬프트 구성: positive(neck, shoulders, mid-torso, lower-torso), negative(nose, eyes, ears + 반팔 시 wrists). multimask_output=True로 3개 마스크 후보 중 최고 점수 선택. 시맨틱 레이블 없으므로 후처리(hood/scarf merge, hull pre-filter, hair exclude) 불필요 → CONSENSUS_MODE="off", DILATION_ITER≤1 자동 오버라이드.
- **[v4] SAM2 Garment-adaptive arms** — SAM2 모드에서도 SegFormer 1-shot으로 가먼트 소매 감지 후 INCLUDE_ARMS 결정. 긴팔→wrist를 negative에서 제거(팔 포함), 반팔→wrist를 negative로 추가(팔 제외). SegFormer는 garment 1장만 추론하므로 VRAM/시간 부담 최소.
- **[v5] SegFormer 모드 폐기, SAM2-only** — SegFormer 마스크 엔진은 SAM2 Image Predictor 대비 열등: 시맨틱 레이블 기반이라 배 구멍/외곽 overflow 등 후처리 복잡도 폭증 (30+ 파라미터), 학습 데이터(SAM2+DWPose 마스크)와의 분포 불일치 근본 미해결. MASK_ENGINE 기본값 "sam2"로 변경, "segformer" 옵션 제거. SegFormer 코드 경로는 dead code로 잔존 (향후 삭제 가능).
- **[v5] 성능 최적화 — GPU 범용 (H100/Blackwell/A100)** — (1) scipy→OpenCV 후처리: `binary_dilation`→`cv2.dilate`, `binary_fill_holes`→`cv2.floodFill`(4-corner flood), `binary_closing`→`cv2.morphologyEx`, `ndimage_label`→`cv2.connectedComponents(connectivity=4)`. C++ 최적화로 3-5x 빠름. (2) ThreadPoolExecutor(4 workers) 비동기 I/O: 프레임 저장을 GPU 연산과 겹침. (3) CUDA: `cudnn.benchmark=True`, `allow_tf32=True`, SAM2 ViT를 FP16으로 강제 (`model.half()` + `torch.autocast`). (4) 파이프라인 각 단계 `time.time()` 계측 출력.
- **[v5] DWPose draw_bodypose 직접 호출 실패** — upstream `draw_bodypose(canvas, candidate, subset)` 시그니처가 `(canvas, kps, stickwidth=)` 가 아니라 candidate/subset 2-인자 구조. stickwidth는 함수 내부 하드코딩(=4). 직접 호출 불가 → `detector(pil)` 시각화 유지 + `inference_detector()+pose_estimation()` 별도 호출로 bbox/kps 추출. ONNX 4회/frame 유지 (절약 불가).
- **[v5] cv2_fill_holes 버그 수정** — 초기 구현이 (0,0)에서만 floodFill → 마스크가 모서리에 닿으면 배경 미도달 → 전체를 hole로 오인 → 마스크 깜빡임. 수정: 4개 꼭짓점 모두에서 flood + invert 로직 정정. cv2_label도 scipy 기본과 동일한 4-connectivity로 수정.
- **[v5] SAM2 모드 DILATION_ITER 제한 제거** — 기존 `min(DILATION_ITER, 1)` 강제 cap이 마스크 테두리 부족 유발. Colab form 슬라이더 값 그대로 사용하도록 변경 (기본 3).
- **[v5] 추론 모델 캐싱 — Pipeline Transplant** — H-3를 subprocess(`!python inference.py`) → 인라인 `TryOnController` 실행으로 전환. **핵심 전략**: (1) inference.py 패치를 디스크에 적용 후 `importlib` reimport → 패치 반영, (2) 이전 실행의 `TryOnVideoPipeline` 객체(GPU 상주)를 `globals()`에서 추출, (3) 새 controller에 pipeline 이식(`_swifttry_controller.pipeline = _cached_pipeline`). tryon_video() 내부 `if self.pipeline is None:` 체크로 모델 로딩 스킵. **clip_length=10000** (upstream `__main__`과 동일) → 내부 clamp로 전체 프레임 처리. 슬라이딩 윈도우는 `TryOnVideoPipeline.__call__()` 내부 context_scheduler(context_frames=16, random_shift, ShiftCaching)가 처리하므로 외부 루프 불필요. **1차 실패 원인**: clip_length를 16으로 설정하여 1 clip만 처리됨 (비디오 잘림). CWD를 `/content/SwiftTry`로 전환 후 `finally`로 복원(config 내 상대 경로 해석). 첫 실행: ~20초 모델 로딩 + 추론, 재실행: <1초 setup + 추론만.
- **[v5] Colab GPU: RTX PRO 6000 Blackwell (96GB GDDR7)** — Colab G4 tier. 메모리 대역폭 ~1.8-2.0 TB/s (A100 HBM2e 2.0 TB/s와 유사), Blackwell 5세대 Tensor Core (compute 우위). SAM2 encoding은 bandwidth-bound라 A100 수준, diffusion 추론(compute-bound)은 30-40% 빠름.
