...
2. POST `{ "data": [<input>] }` to `${ {BACKEND_ENV_VAR} }/run/predict`.
3. Read `response.data[0]` as the AI output. Asset MIME: `{ASSET_MIME}`.
...
public class {RECIPE_NAME}Controller {
...
2. Tag the AI repo (e.g. `{RECIPE_NAME}-v{RECIPE_VERSION}`)
